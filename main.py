...
def update_response_by_id(response_id):
    ...
    print(mark_tag[-1].text.strip('\n\t'), int(soup.find('span', tag_num_views).next_element),int(soup.find('span', tag_num_comments).text))
    __response2 = session.query(Response).filter_by(response_id=response_id).first()
    setattr(__response2, 'last_update', datetime.now())
    setattr(__response2, 'status', mark_tag[-1].text.strip('\n\t'))
    setattr(__response2, 'num_views', int(soup.find('span', tag_num_views).next_element))
    setattr(__response2, 'num_comments', int(soup.find('span', tag_num_comments).text))
    session.commit()
    return


def fetch_response(response_id):
    if session.query(Response.bank_name).filter_by(response_id=response_id).scalar() is not None:
        logging.info(f'in {datetime.now().time()} start upd id {response_id}')
        update_response_by_id(response_id)
        logging.info(f'in {datetime.now().time()}  ended upd id {response_id}')
    else:
        logging.info(f'in {datetime.now().time()}  start get id {response_id}')
        get_response_by_id(response_id)
        logging.info(f'in {datetime.now().time()}  ended get id {response_id}')


engine = create_engine(db_string, pool_recycle=60)
Session = sessionmaker(bind=engine)
session = Session()
new_ids = [result.response_id for result in session.query(Response.response_id).filter_by(**{'bank_name': None})]
